...
def create_list_with_audio_filepaths_in_folder(folder_path: str):
    ...
    for dirpath, dirnames, filenames in os.walk(folder_path):
        audio_filepaths += [
            os.path.join(dirpath, file) for file in filenames if file.endswith(".flac")
        ]
...
def open_flac_file(
    end_time, file_path: str, start_time: int = 0
    ...
    audio, sample_rate = soundfile.read(file_path, start=start_time, stop=end_time)
...
def save_audio_file_start_end(
    ...
    soundfile.write(
        f"{output_path}//{str(uuid.uuid4())}_{index}.flac", audio, sample_rate
    )


def create_slice_from_flac(
    file_path: str, slice_len: int, step_size: float, start_time: int, end_time
...
def create_spec_from_slice_array(
    slices: np.ndarray, steps: int, spec_window, NFFT, samp_freq
...
def create_mfcc_from_slice_array(
    slices: np.ndarray, steps: int, spec_window, N_MFCC, samp_freq
):
...
def create_xarray_dataset_from_dic(
    dic: dict, freqs_spec: np.ndarray, times: np.ndarray
) -> xr.DataArray:
    ...
    for key, fft_slice in dic.items():
        slices_combined[key] = xr.DataArray(
            fft_slice,
            dims=("freq", "times"),
            coords={"freq": freqs_spec, "times": times},
        )
...
def create_xarray_dataset_from_mfcc(
    dic: dict, freqs_spec: np.ndarray, times: np.ndarray
) -> xr.DataArray:
    """Creates an xarray.Dataset object from a dictionary input."""
...
def compute_spectral_features(Dataset: xr.DataArray) -> pd.DataFrame:
    ...
    spec_power = (
        Dataset.groupby("slices")
        .sum(xr.ALL_DIMS)["__xarray_dataarray_variable__"]
        .values
    )
    ...
    for value in Dataset["slices"].values:
        spec_pur = (
            stats.gmean(
                Dataset.sel(slices=value)["__xarray_dataarray_variable__"].values,
                axis=None,
            )
            / Dataset.sel(slices=value)[  # type: ignore
                "__xarray_dataarray_variable__"
            ].values.mean()
        )

        mag_array = (
            Dataset["__xarray_dataarray_variable__"]
            .sel(slices=value)
            .max(dim="times")
            .values
        )
        ...
        slope, _, _, _, _ = stats.linregress(freq_array, freq_mag)
        spec_ro = 0.95 * sum(freq_mag)
...
def process_audio_MFCC(
    ...
    slices_Dataset = pd.DataFrame.from_dict(
        spec_slices, orient="index", columns=["mfcc", "mfcc_delta", "mfcc_delta2"]
    )
...
def get_site_id_from_audio_file(audio_file_path: str):
    """Get site id from audio file
    ...
    """
    # split on \ and split on _ and take second to last and third to last element
    site_id = audio_file_path.split("\\")[-1].split("_")[0:2]
    # join the two elements
...
def create_pandas_dataframe_from_audio_files(
    list_with_audio_files: list,
) -> pd.DataFrame:
...
def select_audio_files(
    config: Config, timestamp: pd.Timestamp, site_id: str, audio_files: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    audio_files = audio_files.query("timestamp >= @start_time & timestamp <= @end_time")
    # sort on timestamp
...
def select_random_audio_point(existing_timestamp: pd.Timestamp) -> pd.Timestamp:
    ...
    start_time = existing_timestamp - timedelta(
        minutes=60
    )  # One hour before existing timestamp
    end_time = existing_timestamp + timedelta(
        minutes=60
    )  # One hour after existing timestamp
...
def create_audio_from_agouti_annotations(
    config: Config, agouti: pd.DataFrame, audio_files: pd.DataFrame
):
    ...
    for index, row in tqdm(agouti.iterrows(), total=agouti.shape[0]):
        ...
        selected_audio_files = select_audio_files(
            config, timestamp, site_id, audio_files
        )
        random_selected_audio_files = select_audio_files(
            config, random_timestamp, site_id, audio_files
        )
        ...
        if not audio_files.empty:
            ...
            audio.export(
                config.output_path
                + "\\rats"
                + f'\\{site_id}_{timestamp.strftime("%Y_%m_%d_%H_%M")}.flac',
                format="flac",
            )
            random_audio.export(
                config.output_path
                + "\\noise"
                + f'\\{site_id}_{random_timestamp.strftime("%Y_%m_%d_%H_%M")}.flac',
                format="flac",
            )


def create_audio_from_agouti_and_arise_data(config: Config, agouti: pd.DataFrame):
    arise_df_path = r"G:\thesis\ThesisRodentClassification\pandas_df_arise_api_gijs_data_request.pkl"
...
def test():
    ...
    agouti["timestamp"] = pd.to_datetime(
        [
            "2022-05-28 07:08:09+01:00",
            "2022-05-28 11:08:09+01:00",
            "2022-05-28 18:08:09+01:00",
        ]
    )
    agouti["fileName"] = [
...
def test1():
    config = Config()
    agouti = load_agouti_data(config)
    create_audio_from_agouti_and_arise_data(config, agouti)
